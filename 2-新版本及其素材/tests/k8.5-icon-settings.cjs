...
const resources = path.resolve(__dirname, '../发行版/K8.5/win-unpacked/resources');
app.setPath('userData', fs.mkdtempSync(path.join(os.tmpdir(), 'zhijian-icon-settings-')));
let fail = false;
app.whenReady().then(async () => {
  let win;
  try {
    for (const [name, value] of Object.entries({ 'get-version': '0.10.5', 'get-system-theme': false,
    ...
    for (const name of ['main.js', 'windows-icons.js', 'preload.js']) {
