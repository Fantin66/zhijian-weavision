...
(function(root){
  ...
  function decode(structure,attachments,next){
    ...
    if(structure.schemaVersion&&structure.schemaVersion>2)throw new Error("此文件来自较新版本，请先升级织见");
    ...
    for(const f of structure.fileMeta||[]){
      ...
      if(!a&&!(f.kind==="link"&&f.url))throw new Error("附件缺失："+f.name);
      const blob=a?new Blob([a.buffer],{type:f.mime||"application/octet-stream"}):null;
      fm.set(f.oldId,id);p.files.push({id,name:f.name,kind:f.kind,mime:f.mime||"",url:f.url||null,size:blob?.size||0,blob,created:f.created||Date.now(),folderId:folders.get(f.folderId)||null,thumb:null,tw:1,th:1});
    }
    ...
    for(const c of structure.canvases){
      ...
      copy.links=(copy.links||[]).map(l=>{l.id=next("lnk");l.aId=m.get(l.aId);l.bId=m.get(l.bId);if(l.sourceRef?.fileId)l.sourceRef.fileId=fm.get(l.sourceRef.fileId)||null;return l;});
