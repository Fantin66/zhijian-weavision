...
function hitHandle(sel,sx,sy){
  ...
  /* 命中判定适度放宽（用户反馈：原 9px 圆形判定太严，鼠标必须精确落在角点/边中点的
     那一条线上才触发）。改为"一块区域"判定——角手柄=方形区域、边手柄=带状区域，
     向元素内收进一小段仍可命中。注意：只放宽到"更容易选中"即可，不宜过大以免误触。 */
  const CORNER=18;  /* 角手柄：方形半宽（屏幕像素） */
  const BAND=13;    /* 边手柄：法向厚度（往元素内收一小段） */
  const EXT=20;     /* 边手柄：切向延伸半长（沿边方向放宽） */
